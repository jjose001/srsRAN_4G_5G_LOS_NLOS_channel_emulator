...
void slot_sync::set_dl_channel(srsran::channel* dl_channel_)
{
  dl_channel = dl_channel_;
}
...
int slot_sync::recv_callback(srsran::rf_buffer_t& data, srsran_timestamp_t* rx_time)
{
  ...
  *rx_time = rf_timestamp.get(0);
  ...
  // Execute DL channel emulator (in-place processing)
  if (dl_channel) {
    dl_channel->set_srate((uint32_t)ue_sync_nr.srate_hz);
    dl_channel->run(data.to_cf_t(), data.to_cf_t(), data.get_nof_samples(), *rx_time);
  }
