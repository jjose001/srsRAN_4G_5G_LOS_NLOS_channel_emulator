...
bool sync_sa::init(const args_t& args, stack_interface_phy_nr* stack_, srsran::radio_interface_phy* radio_)
{
  ...
  if (args.ul_channel_args.enable) {
    uint32_t nof_channels = args.nof_rx_channels;
    
    logger.info("=== Initializing UL channel emulator for NR: channels=%d, AWGN=%s, Fading=%s (model=%s) ===",
                nof_channels,
                args.ul_channel_args.awgn_enable ? "enabled" : "disabled",
                args.ul_channel_args.fading_enable ? "enabled" : "disabled",
                args.ul_channel_args.fading_model.c_str());
    
    ul_channel = srsran::channel_ptr(
        new srsran::channel(args.ul_channel_args, nof_channels, logger));
    
    if (ul_channel) {
      ul_channel->set_srate((uint32_t)args.srate_hz);
      logger.info("UL channel emulator initialized successfully with sample rate: %.2f MHz", args.srate_hz / 1e6);
    } else {
      logger.error("Failed to initialize UL channel emulator");
    }
  }

  // Instantiate DL channel emulator
  if (args.dl_channel_args.enable) {
    uint32_t nof_channels = args.nof_rx_channels;
    
    logger.info("=== Initializing DL channel emulator for NR: channels=%d, AWGN=%s, Fading=%s (model=%s) ===",
                nof_channels,
                args.dl_channel_args.awgn_enable ? "enabled" : "disabled",
                args.dl_channel_args.fading_enable ? "enabled" : "disabled",
                args.dl_channel_args.fading_model.c_str());
    
    dl_channel = srsran::channel_ptr(
        new srsran::channel(args.dl_channel_args, nof_channels, logger));
    
    if (dl_channel) {
      dl_channel->set_srate((uint32_t)args.srate_hz);
      logger.info("DL channel emulator initialized successfully with sample rate: %.2f MHz", args.srate_hz / 1e6);
      
      // Pass DL channel to slot synchronizer (raw pointer, ownership remains in sync_sa)
      slot_synchronizer.set_dl_channel(dl_channel.get());
    } else {
      logger.error("Failed to initialize DL channel emulator");
    }
  }
                         ...
                         srsran::rf_buffer_t&                                  tx_buffer)
{
  ...
  if (tx_enable) {
    // Run uplink channel emulator (in-place processing)
    if (ul_channel) {
